...
VERSION_FILE := $(INCLUDE_DIR)/fwd_version.h
get_version = $(shell grep -E '^ *\#define +FWD_VERSION_$1 +[0-9]+$$' $(VERSION_FILE) | sed 's/  */ /g' | cut -d ' ' -f 3)
...
PKGS = glib-2.0 gio-2.0 gobject-2.0
...
SRC := \
  fwd_control_client.c \
  fwd_control_service.c \
  fwd_entry.c \
  fwd_entry_datagram_connection.c \
  fwd_entry_datagram_forward_local.c \
  fwd_entry_datagram_forward_remote.c \
  fwd_entry_datagram_listen.c \
  fwd_entry_datagram_socket.c \
  fwd_entry_stream_forward_local.c \
  fwd_entry_stream_forward_remote.c \
  fwd_entry_stream_listen.c \
  fwd_entry_stream_socket.c \
  fwd_op.c \
  fwd_op_call.c \
  fwd_op_connect_local.c \
  fwd_op_connect_remote.c \
  fwd_socket_client.c \
  fwd_socket_io.c \
  fwd_socket_service.c \
  fwd_peer.c \
  fwd_util.c
...
clean:
	...
	rm -fr debian/.debhelper debian/libfwd debian/libfwd-dev debian/libfwd-tools
	rm -f documentation.list debian/files debian/*.substvars
	rm -f debian/*.debhelper.log debian/*.debhelper debian/*~
	rm -f debian/libfwd.install debian/libfwd-dev.install
...
$(DEBUG_SO): $(DEBUG_OBJS)
	$(LD) $(DEBUG_OBJS) $(DEBUG_LDFLAGS) -o $@ $(DEBUG_LIBS) $(LIBS)

$(RELEASE_SO): $(RELEASE_OBJS)
	$(LD) $(RELEASE_OBJS) $(RELEASE_LDFLAGS) -o $@ $(RELEASE_LIBS) $(LIBS)
ifeq ($(KEEP_SYMBOLS),0)
	$(STRIP) $@
endif
